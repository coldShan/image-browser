...
- `画集模式`：按一级子目录与二级子目录聚合为画集；若当前目录本身有图片，则额外生成一个“当前目录”画集。一级子目录画集只包含直属图片；二级子目录会与其父级并列展示，并继续归集其更深层图片。
...
- 预览 URL 采用字节软上限缓存和回收机制；离开视口后延迟 3 分钟释放。
...
- 使用 `localStorage` 记录最近阅读状态：包含最后查看图片位置、最近打开图集与图集进度（按最后位置占比）。
- 全图模式与画集模式阅读进度独立维护，互不同步（全图记录不会覆盖画集进度，反之亦然）。
- 重新打开同一来源后会自动恢复滚动位置：全图列表、画集列表与画集详情均可定位到上次查看位置（若路径不存在会回退索引）。
